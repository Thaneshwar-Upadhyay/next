  ...
  <div className="w-full max-w-md space-y-8">
    <div>
      ...
      <p className="mt-2 text-center text-sm text-gray-600">
        Or
        <Link href={'/signup'} legacyBehavior><a href="#" className="font-medium text-pink-600 hover:text-pink-500 ml-2">Sign Up</a></Link>
      </p>
    </div>
    <form className="mt-8 space-y-6" action="#" method="POST">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm">
          <Link href={'/forgot'} legacyBehavior><a href="#" className="font-medium text-pink-600 hover:text-pink-500">Forgot your password?</a></Link>
        </div>
      </div>
      ...
      <div>
        <button type="submit" className="group relative flex w-full justify-center rounded-md border border-transparent bg-pink-600 py-2 px-4 text-sm font-medium text-white hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2">
          <span className="absolute inset-y-0 left-0 flex items-center pl-3">
            <svg className="h-5 w-5 text-pink-500 group-hover:text-pink-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" ariaHidden="true">
